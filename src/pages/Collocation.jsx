...
export function Collocation() {
  ...
  const chargerAnnonces = (quartier = "") => {
    setLoading(true)
    const url = quartier
      ? `annonce/annonces/rechercher/?quartier=${quartier}`
      : `annonce/annonces/filtrer/?type=colocation`  // ← filtre colocation
    API.get(url)
      .then((res) => {
  ...
  return (
    <>
            ...
            {profil && (
              <div style={{
                ...
                <div style={{ textAlign: "center", marginBottom: "12px" }}>
                  ...
                  }}>{user?.first_name?.[0]?.toUpperCase() || "H"}</div>
                </div>
      ...
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center", marginTop: "30px", padding: "0 20px" }}>
        <input
          type="text"
          className="form-control"
          placeholder="Rechercher une collocation par quartier..."
          value={recherche}
      ...
      <div className="sugg">
        <h3 style={{ margin: "30px" }}>Collocations disponibles 🏠</h3>
        <Link to ="/Modal">
        {loading ? (
          <p>Chargement...</p>
        ) : annonces.length === 0 ? (
          <p>Aucune collocation trouvée.</p>
        ) : (
